...
st.set_page_config(page_title="Triple Sapo - Móvil", layout="wide")

# Refresco cada 1 segundo
st_autorefresh(interval=1000, key="reloj_global")

# --- ESTILOS CSS AJUSTADOS PARA CELULARES ---
st.markdown("""
    <style>
    .stApp { background-color: #1a3c1a; color: white; text-align: center; }
    
    /* Títulos reducidos para móvil */
    .titulo { font-size: 42px !important; color: #ccff00; font-weight: bold; margin-bottom: 0px; text-shadow: 2px 2px 8px #000; }
    .subtitulo { font-size: 16px; color: #ffff00; margin-top: -5px; margin-bottom: 15px; letter-spacing: 2px; }
    
    /* Contador reducido para que quepa en el teléfono */
    .timer-digital { 
    ...
    .res-signo { 
        font-size: 28px; /* Tamaño reducido para móvil */
        color: #ccff00; 
        font-weight: bold; 
    ...
    .espera { font-size: 16px; color: #446644; font-style: italic; margin-top: 15px; }
...
# --- INTERFAZ ---
st.markdown(f"<p style='text-align: right; color: #555; font-size: 12px;'>{ahora.strftime('%I:%M:%S %p')}</p>", unsafe_allow_html=True)
...
st.markdown("<p style='color: #444; font-size: 14px;'>PRÓXIMO SORTEO</p>", unsafe_allow_html=True)
...
for i, h in enumerate(h_labels):
    with cols[i]:
        ...
        # Hora centrada con línea de color
        st.markdown(f"<div class='hora-txt' style='border-bottom: 2px solid {colores_h[i]};'>{h}</div>", unsafe_allow_html=True)
        ...
        if res:
            ...
            st.markdown("<div class='espera'>Esperando...</div>", unsafe_allow_html=True)
