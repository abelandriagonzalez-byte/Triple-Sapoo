...
st.markdown("""
    ...
    .titulo-top { font-size: 55px !important; color: #ccff00; font-weight: bold; margin-bottom: 0px; text-align: left; }
    .subtitulo-top { font-size: 22px; color: #ffff00; margin-top: -10px; text-align: left; letter-spacing: 2px; }
    .timer-digital { font-size: 65px !important; color: #ffffff; font-family: monospace; font-weight: bold; text-align: left; margin-top: 10px; }
...
# --- CABECERA CON IMAGEN A LA DERECHA ---
col_texto, col_imagen = st.columns([2, 1])

with col_texto:
    st.markdown("<div class='titulo-top'>TRIPLE SAPO</div>", unsafe_allow_html=True)
    st.markdown("<div class='subtitulo-top'>LOTERÍA DE MARA</div>", unsafe_allow_html=True)
    ...
    st.markdown("<p style='text-align: left; color: #444; font-size: 14px; margin-top:-5px;'>PRÓXIMO SORTEO EN</p>", unsafe_allow_html=True)

with col_imagen:
    try:
        st.image("logo.png", width=220)
    except:
        st.write("Carga logo.png en GitHub")
...
for idx, h_m in enumerate(horarios_metas):
    ...
    if ahora > (t_s + timedelta(seconds=10)) and not obtener_datos_dia(fecha_hoy, h_labels[idx]):
        a = f"{random.randint(0,999):03d}"
        b = f"{random.randint(0,999):03d}"
        c = f"{random.randint(0,999):03d}"
